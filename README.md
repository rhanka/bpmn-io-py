...
flow = moddle.create("bpmn:SequenceFlow", {"id": "F1", "sourceRef": start, "targetRef": task})
process.get("flowElements").extend([start, task, flow])
definitions.get("rootElements").append(process)

print(moddle.to_xml(definitions, {"format": True}).xml[:120])
# <?xml version="1.0" encoding="UTF-8"?>
# <bpmn:definitions ... id="D" ...><bpmn:process id="Hiring" isExecutable="true">...
```

Sketch of the tree you just built (`Apply ─F1─▶ Screen`, both contained in `Hiring`):
...
└── Process "Hiring"
    ├── StartEvent "Apply"
    ├── Task "Screen"
    └── SequenceFlow "F1" (Apply → Screen)
```

### 2. Lint a file (integrity check)
...
report = check(BpmnModdle().from_xml(xml).root_element)  # model level: strict
print(report.ok, report.errors)
# False ['unknown attribute <bogus>']
```

`check` is the L0 integrity gate (unparsable input, unknown attributes, dangling
references) — the same findings the CLI reports with exit code 1. Semantic
business rules (bpmnlint-style) are out of scope for v1.

### 3. Lay out a diagram-less file
...
### 4. Render it with bpmn-js

The Python lib does the modelling; [bpmn-js](https://bpmn.io/toolkit/bpmn-js/)
does the pixels. `docs/examples/` ships a ready-made pair:

- `example.bpmn` — a hiring process generated by `bpmn-io` itself (same
  `create` calls as §1 above, finished with `layout_process`);
- `viewer.html` — a minimal viewer (bpmn-js Viewer from CDN, file picker
  included).

```bash
cd docs/examples && python3 -m http.server 8080
# open http://localhost:8080/viewer.html
```

The viewer fetches `example.bpmn` by default; drop any `.bpmn` produced by the
`bpmn-io` CLI (`bpmn-io layout …`) into the picker to render your own files.
