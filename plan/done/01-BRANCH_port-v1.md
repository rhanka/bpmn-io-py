...
- `blocked` BR01-PR: draft PR `port-v1` → `main` and `git push` pending: origin already points at `rhanka/bpmn-io-py`; verified 2026-09-24 (tree clean, 37 commits ahead of origin/port-v1, fast gates green) that `gh auth status` fails — token in `~/.config/gh/hosts.yml` invalid (401), owner re-auth via `gh auth login -h github.com` needed before push. Recorded per BRIEF rule 8; unblocks the Lot 0 checkbox.
